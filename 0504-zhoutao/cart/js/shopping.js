function checkItem(item) {
    console.log(item);
    let line = item.parentNode.parentNode;
    let id = line.id.split('-')[1]
    line.style.backgroundColor = '';
    if (item.checked) {
        line.style.backgroundColor = '#fff4e8';
    }
    let line_price = document.getElementsByClassName('item-price-' + id + ' unit');
    let count = parseFloat(document.getElementsByClassName('count-' + id)[1].previousElementSibling.textContent)
    let price = parseFloat(line_price[0].textContent.slice(1)) * count;
    ...
    if (item.checked) {
        ...
        let btn_all_botton = document.getElementById('check-all-botton');
        btn_all_botton.checked = false;
    }
    document.getElementsByClassName('total-price')[0].textContent = '￥' + total_price.toFixed(2)

}

function checkAll_botton() {
    let all = document.getElementsByName('item-check');
    let btn_all = document.getElementById('check-all');
    let btn_all_botton = document.getElementById('check-all-botton');
    let shop_all = document.getElementsByClassName('shop');
    document.getElementsByClassName('total-price')[0].textContent = '￥0.00';
    for (let a of all) {
        for (let shop of shop_all) {
            shop.firstElementChild.firstElementChild.checked = btn_all.checked;
        }
        btn_all.checked = btn_all_botton.checked
        a.checked = btn_all_botton.checked;
        if (a.checked) {
            checkItem(a);
        } else {
            let line = a.parentNode.parentNode;
            line.style.backgroundColor = '';
        }
    }
}
...
function checkAll() {
    ...
    for (let a of all) {
        ...
        btn_all_botton.checked = btn_all.checked;
...
function deleteChecked(bAll = false) {
    ...
    let btn_all = document.getElementById('check-all');
    let btn_all_botton = document.getElementById('check-all-botton')
    if (btn_all.checked || bAll || btn_all_botton.checked) {
        let by_all = document.getElementById('buy-all');
        by_all.remove();
    } else {
        for (var i = all.length - 1; i >= 0; i--) {
            if (all[i].checked) {
                let id = all[i].id.split('-')[2];
    ...
    document.getElementById('check-all-botton').checked = false
    document.getElementsByClassName('total-price')[0].textContent = '￥0.00';

}

function deleteOther(item) {
    let line = item.parentNode;
    line.previousElementSibling.remove();
    line.parentNode.remove();
}

function shopAll(item) {
    let name = item.name.split('-')[1];
    let all_shop = document.getElementsByClassName('item-check-' + name);
    for (let a of all_shop) {
        a.checked = item.checked;
        checkItem(a);
    }
}

function littlePrice(item) {
    let id = item.className.split('-')[1];
    let value = item.previousElementSibling;
    let price = document.getElementsByClassName('item-price-' + id + ' unit')[0].textContent.slice(1);
    if (value) {
        var count = parseFloat(value.textContent)
        count += 1;
        item.previousElementSibling.textContent = String(count);
    } else {
        var count = parseFloat(item.nextElementSibling.textContent);
        count -= 1;
        if (count == 0) {
            confirm("购买数量最小为1")
            count = 1;
        }
        item.nextElementSibling.textContent = String(count);
    }
    document.getElementsByClassName('item-price-all-' + id + ' little-total')[0].textContent = '￥' + (parseFloat(price) * parseFloat(count)).toFixed(2);
    let all = document.getElementsByName('item-check');
    document.getElementsByClassName('total-price')[0].textContent = '￥0.00';
    for (let a of all) {
        if (a.checked) {
...
function myChoice() {
    var x;
    let total_price = document.getElementsByClassName('total-price')[0].textContent
...
var prov_select = document.querySelector('#add_p');
var city_select = document.querySelector('#add_c');
var country_select = document.querySelector('#add_a');
provice.forEach(function choiceCity(val, index) {
    prov_select.add(new Option(val.name, index));
});
...
function prov(item) {
    ...
    provice[index].city.forEach(function (val, index) {
        city_select.add(new Option(val.name, index));
    })
    return index
}

function city(item) {
    let new_index = item.value;
    let index = item.previousElementSibling.value;
    country_select.length = 0;
    provice[index].city[new_index].districtAndCounty.forEach(function (val, index){
        country_select.add(new Option(val, index))
    });
